...
def parser_equatorial(texto: str) -> Dict:
    """
    Parser robusto para faturas do grupo Equatorial: PA, MA, PI, AL, GO, RS.
    """
    ...
    for estado in ["PA", "MA", "PI", "AL", "GO", "RS"]:
        if f"equatorial {estado.lower()}" in texto.lower():
            distribuidora = f"Equatorial {estado}"
            break
